...
class CustomEnv:
    ...
    def __init__(self, df, initial_balance=1000, lookback_window_size=50, Render_range=100, Show_reward=True, Show_indicators=False, normalize_value=40000):
...
if __name__ == "__main__":            
    ...
    csv_name =  './EURUSD_1h_5months.csv'
    ...
    def testmonth(wish):
        if wish == 1:
            test_df = df[-test_window-lookback_window_size:  - 720 ]
            test_df_nomalized = df_nomalized[-test_window-lookback_window_size: - 720]
            month_tested = "first month"
        if wish == 1.1:
            test_df = df[-test_window-lookback_window_size:  - 720 - half]
            test_df_nomalized = df_nomalized[-test_window-lookback_window_size:  - 720 - half]
            month_tested = "first month 1st half"
        if wish == 1.2:
            test_df = df[- half -test_window-lookback_window_size:  - 720 ]
            test_df_nomalized = df_nomalized[- half -test_window-lookback_window_size:  - 720 ]
            month_tested = "first month 2nd half"

        elif wish == 2:
            test_df = df[-  720-lookback_window_size: ]
            test_df_nomalized = df_nomalized[-720-lookback_window_size:]
            month_tested = "second month" 

        elif wish == 3:
            test_df = df[-test_window-lookback_window_size:  ]
            test_df_nomalized = df_nomalized[-test_window-lookback_window_size: ]
            month_tested = "all month" 

        elif wish == "all":
            test_df = df
            test_df_nomalized = df_nomalized
            month_tested = "all month" 

        return test_df , test_df_nomalized , month_tested
    ...
    train_df = df[100:] # we leave 100 to have properly calculated indicators
    test_df = df[-test_window-lookback_window_size:]
    wish = "all"
    # test_df , test_df_nomalized , month_tested = testmonth(wish)
